...
### 20. PATCH `/api/v1/wallets/{walletId}/default`

**Purpose:** Set a specific wallet as the default wallet for the user

**Used In:**

- `lib/api-client.ts` - `wallets.setDefaultWallet()` function
- `lib/wallet-context.tsx` - `setDefaultWallet()` function
- `components/wallet/wallet-modal.tsx` - Wallet selection and default setting

**Feature Description:**
The set default wallet feature allows users to:

- Choose their preferred wallet for playing games
- Set which wallet should be used for deposits and withdrawals by default
- Quickly switch between different currency wallets
- Designate the primary wallet displayed in the header

When a wallet is set as default:

- The `isDefault` flag is set to `true` for the selected wallet
- All other wallets have `isDefault` set to `false`
- The default wallet is highlighted in the wallet list
- Balance in the header updates to show the default wallet's balance
- Games will use this wallet for betting by default

**URL Parameters:**

- `walletId` (string): The ID of the wallet to set as default

**Request Body:** (Empty - wallet ID is in URL)
...
```typescript
{
  ...
  data: {
    ...
    balance: number;
    bonusBalance: number;
    isDefault: boolean; // Will be true for this wallet
    isActive: boolean;
    createdAt: string;
    updatedAt: string;
  }
...
1. Log in to the application
2. Ensure you have multiple wallets (if not, create additional wallets)
3. Click on the wallet balance in the header
4. Wallet modal opens showing all your wallets
5. Find a wallet that is NOT currently the default
6. Click the "Set as Default" button or star icon on that wallet
7. Verify that:
   - Success toast notification appears
   - The selected wallet is now marked as default (highlighted or with badge)
   - Previous default wallet no longer shows as default
   - Header balance updates to show the new default wallet's balance
   - Change persists after page reload
   - Only one wallet has `isDefault: true` at a time

**Test with Multiple Wallets:**

1. Create wallets in different currencies (EUR, USD, BTC, etc.)
2. Switch default between them
3. Verify each switch:
   - Updates the default correctly
   - Shows correct currency symbol in header
   - Maintains correct balance for each wallet
   - No data loss or corruption

**Test Error Handling:**

1. Try setting a non-existent wallet as default (manipulate API call)
2. Verify appropriate error message
3. Try setting another user's wallet as default (if possible)
...
function MyComponent() {
  const { wallets, setDefaultWallet, loading } = useWallet();
    ...
    <div>
      {wallets?.map((wallet) => (
        <div key={wallet.id}>
          ...
          {wallet.isDefault && <span>★ Default</span>}
        </div>
      ))}
    </div>
  );
...
### 21. GET `/api/v1/coin-networks/active`
...
### 25. PATCH `/api/v1/notifications/{id}/read`
...
### 26. PATCH `/api/v1/notifications/read-all`
...
### 28. DELETE `/api/v1/sessions/{sessionId}`
...
- **4 Wallet Endpoints** - Wallet management, balance tracking, transaction history, and default wallet setting
