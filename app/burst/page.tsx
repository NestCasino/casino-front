...
export default function BurstPage() {
  ...
  const burstCategory = getCategoryBySlug('burst') || getCategoryBySlug('crash')
  
  const { games, meta, loading, error } = useGames({
    categoryId: burstCategory?.id,
    providerId,
    search: searchTerm,
    page: currentPage,
    perPage: 24,
  ...
  useEffect(() => {
    setCurrentPage(1)
    setAllGames([])
  }, [burstCategory?.id, providerId, searchTerm])
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">
                {meta ? `${meta.total} crash and burst games` : 'Loading...'}
              </p>
            </div>
          </div>
