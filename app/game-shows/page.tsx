...
export default function GameShowsPage() {
  ...
  const gameShowsCategory = getCategoryBySlug('game-shows') || getCategoryBySlug('gameshows')
  
  const { games, meta, loading, error } = useGames({
    categoryId: gameShowsCategory?.id,
    providerId,
    search: searchTerm,
    page: currentPage,
    perPage: 24,
  ...
  useEffect(() => {
    setCurrentPage(1)
    setAllGames([])
  }, [gameShowsCategory?.id, providerId, searchTerm])
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">
                {meta ? `${meta.total} interactive game shows` : 'Loading...'}
              </p>
            </div>
          </div>
