...
import { Spade, ChevronDown } from 'lucide-react'
...
export default function BlackjackPage() {
  ...
  const blackjackCategory = getCategoryBySlug('blackjack')
  ...
  useEffect(() => {
    setCurrentPage(1)
    setAllGames([])
  }, [blackjackCategory?.id, providerId, searchTerm])
      ...
      <main className={cn(
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center gap-3 mb-8">
            <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-gray-700 to-gray-900">
              <Spade className="h-6 w-6 text-white" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-white">Blackjack</h1>
              <p className="text-gray-400 text-sm mt-1">
                {meta ? `${meta.total} blackjack games` : 'Loading...'}
              </p>
            </div>
          </div>
