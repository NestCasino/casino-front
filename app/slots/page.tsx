...
import { Cherry, ChevronDown } from 'lucide-react'
...
export default function SlotsPage() {
  ...
  const slotsCategory = getCategoryBySlug('slots')
  ...
  useEffect(() => {
    setCurrentPage(1)
    setAllGames([])
  }, [slotsCategory?.id, providerId, searchTerm])

  // Accumulate games when new page loads
  useEffect(() => {
    if (games && games.length > 0) {
      setAllGames(prev => {
        // If it's page 1, replace all games
        if (currentPage === 1) {
          return games
        }
      ...
      <main className={cn(
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center gap-3 mb-8">
            <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-red-500 to-pink-600">
              <Cherry className="h-6 w-6 text-white" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-white">Slots</h1>
              <p className="text-gray-400 text-sm mt-1">
                {meta ? `${meta.total} exciting slot games` : 'Loading...'}
              </p>
            </div>
          </div>
